...
def get_cc_co_ci_by_contract_address(contract_address: str) -> Tuple[str, str, str]:
    r = requests.post("http://0.0.0.0:" + str(consts.MINER_SERVER_PORT) + "/get_cc_co_ci", data=compress(contract_address))
    data = r.json()
    return data.get('cc', ''), data.get('co', ''), data.get('ci', '')
...
class BlockchainVMInterface:
    ...
    def read_contract_output(self, contract_address: str) -> Optional[str]:
        ...
        _, co, _ = get_cc_co_ci_by_contract_address(contract_address)
    ...
    def call_contract_function(self, contract_address: str, function_name: str, params: List[Any]) -> Optional[str]:
        if not is_valid_contract_address(contract_address):
            raise Exception(f"Contract Address {contract_address} is invalid contract address")
        cc, _, ci = get_cc_co_ci_by_contract_address(contract_address)
        if cc == '':
            return None
        else:
            return self.__run_function(cc, function_name, params, ci)
    ...
    def __run_function(self, code: str, function_name: str, params: List[Any], contract_id: str) -> Optional[str]:
    ...
    def run_contract_code(self, contract_code: str, contract_id: str) -> Optional[str]:
        return self.__run_function(contract_code, "main", [], contract_id)
